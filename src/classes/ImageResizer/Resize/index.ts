export class Resize {
  ...
  widthBuffer?: Float32Array | number[];
  outputWidthWorkBench?: Float32Array | number[];
  outputWidthWorkBenchOpaquePixelsCount?: Float64Array | number[];

  heightBuffer?: Uint8Array | number[];
  outputHeightWorkBench?: Float32Array | number[];
  outputHeightWorkBenchOpaquePixelsCount?: Float64Array | number[];
  ...
  private _resizeWidthRGBChannels(buffer: Buffer, fourthChannel: boolean) {
    const channelsNum = fourthChannel ? 4 : 3;
    const ratioWeight = this.ratioWeightWidthPass!;
  ...
  private _resizeHeightRGBChannels(buffer: Buffer, fourthChannel: boolean) {
  ...
  private resizeWidthInterpolatedRGB(buffer: Buffer) {
    return this._resizeWidthInterpolatedRGBChannels(buffer, false);
  };
  
  private resizeWidthInterpolatedRGBA(buffer: Buffer) {
    return this._resizeWidthInterpolatedRGBChannels(buffer, true);
  };
  
  private resizeWidthRGB(buffer: Buffer) {
    return this._resizeWidthRGBChannels(buffer, false);
  };
  
  private resizeWidthRGBA(buffer: Buffer) {
    return this._resizeWidthRGBChannels(buffer, true);
  };
  
  private resizeHeightInterpolated(buffer: Buffer) {
    const ratioWeight = this.ratioWeightHeightPass!;
    const outputBuffer = this.heightBuffer!;
  ...
  private resizeHeightRGB(buffer: Buffer) {
    return this._resizeHeightRGBChannels(buffer, false);
  };
  
  private resizeHeightRGBA(buffer: Buffer) {
    return this._resizeHeightRGBChannels(buffer, true);
  };
  
  public resize(buffer: Buffer | Uint8Array) {
    this.resizeCallback(this.resizeHeight(this.resizeWidth(buffer))) as any;
  };
  
  private bypassResizer(buffer: Buffer) {
