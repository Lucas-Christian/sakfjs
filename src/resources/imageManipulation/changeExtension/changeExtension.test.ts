...
describe("#changeExtension", () => {
  let imagePath = `${process.env.TESTIMAGEFOLDER}/image`;
  test("Verifica se todos os erros ocorrem como esperado", async () => {
    try {
      await changeExtension(`${imagePath}2.jpg`, "" as any);
    } catch(error: any) {
      expect(error.message).toBe("unsupportedImage");
    }

    try {
      await changeExtension(`${imagePath}2.jpg`, undefined as any);
    } catch(error: any) {
      expect(error.message).toBe("unsupportedImage");
    }

    try {
      await changeExtension(`${imagePath}2.jpg`, 123 as any);
    } catch(error: any) {
      expect(error.message).toBe("isNotString");
    }

    try {
      await changeExtension(`${process.env.TESTUNSUPPORTEDIMAGEFOLDER}/test.js`, "jpg");
    } catch(error: any) {
      expect(error.message).toBe("pathDoesNotLeadToBuffer");
    }

    try {
      await changeExtension(process.env.TESTUNSUPPORTEDIMAGEFOLDER as string, "jpg");
    } catch(error: any) {
